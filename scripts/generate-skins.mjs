...
/**
 * The skin list, generated as a module.
 *
 * main.js used to carry its own hard-coded copy of the ids and display names.
 * Deleting a skin from skins.json therefore left the cycle button pointing at
 * a skin that no longer existed, and adding one did nothing until someone
 * remembered to edit the second list too. One source of truth instead.
 */
const skinsJs = [
  '// GENERATED by scripts/generate-skins.mjs — do not edit.',
  '// Source of truth: shared/skins.json',
  '',
  `export const SKINS = ${JSON.stringify(source.skins.map((s) => s.id))};`,
  '',
  'export const SKIN_NAMES = {',
  ...source.skins.map((s) => {
    // The short label for the cycle button: the part before the separator.
    const short = (locale) => (s.name[locale] || s.id).split('·')[0].trim();
    return `  ${s.id}: { vi: ${JSON.stringify(short('vi'))}, en: ${JSON.stringify(short('en'))} },`;
  }),
  '};',
  '',
].join('\n');
...
const skinsJsTarget = join(root, 'web', 'src', 'skins.gen.js');

if (checkOnly) {
  let current = '';
  try { current = readFileSync(target, 'utf8'); } catch { /* not generated yet */ }
  let currentJs = '';
  try { currentJs = readFileSync(skinsJsTarget, 'utf8'); } catch { /* not generated yet */ }
  if (current !== css || currentJs !== skinsJs) {
    console.error('skins output is stale — run: node scripts/generate-skins.mjs');
    process.exit(1);
  }
  ...
  mkdirSync(dirname(skinsJsTarget), { recursive: true });
  writeFileSync(skinsJsTarget, skinsJs);
  console.log(`wrote ${target} (${source.skins.length} skins)`);
  console.log(`wrote ${skinsJsTarget}`);
}
